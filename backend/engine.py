...
Phase 2: ML training cadence and P2P negotiation bus.
Phase 3: Async-compatible loop with broadcast callback for WebSockets.
"""
...
class Engine:
    ...
    # ── Async main loop (for FastAPI WebSocket server) ───────────────
    async def run_async(
        self,
        on_tick: Callable[[dict], Awaitable[None]] | None = None,
        max_ticks: int | None = None,
    ) -> None:
        """
        Run the simulation loop as an async coroutine.
        Yields control via asyncio.sleep so FastAPI can serve WebSockets.

        Args:
            on_tick:    optional async callback fired every tick with snapshot
            max_ticks:  stop after N ticks (None = run forever)
        """
        self.running = True
        print(
            f"\n⚡  Microgrid Engine started (async)  "
        ...
        while True:
            ...
            print(
                f"  [{snapshot['tick']:>4}] {snapshot['time']}  "
                f"☀{snapshot['env_solar']:.2f} ⚡{snapshot['env_demand']:.2f}  │  "
                f"{node_summary}"
            )
    ...
    # ── Blocking main loop (for standalone CLI use) ──────────────────
    def run(self, max_ticks: int | None = None) -> None:
        """Run the simulation loop in blocking mode (for CLI testing)."""
        print(
