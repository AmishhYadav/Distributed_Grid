...
- **Global scope**: `/Users/amish/Distributed_Grid/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/amish/Distributed_Grid/.claude/notes/`)
...
2. Glob `/Users/amish/Distributed_Grid/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/amish/Distributed_Grid/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/amish/Distributed_Grid/.claude/notes/` — works in any directory
